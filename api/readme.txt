...
results | Contains the data with inserting the analysis results into it.
        |
...
statics | Contains the statics files for the falsk app
        |
        | ---- css
        | ---- Images
...
Dockerfile --- The docker file for building a docker image to run the app
               (assume that the ElasticSearch runs on localhost:9200 or you can configure the host from config.py file).

requirements.txt --- It contains the needed python libraries for running the app with docker.
